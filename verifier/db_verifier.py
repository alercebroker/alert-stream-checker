...
class DBVerifier:
    def __init__(
        ...
        if len(identifiers) != 2:
            raise ValueError(
                "Identifier list need to have length 2 (oid, candid) from alert."
            )
    ...
    def verify(self) -> Result:
        ...
        while total_messages < self.total_offsets:
            ...
            try:
                ...
                return Result(
                    ...
                    check_success=False,
                    value=None,
                    error=KeyError(f"Key:{str(e)} not found in alert"),
                )
            ...
            try:
                ...
                if total_messages == 0:
                    return Result(
                        success=False, check_success=False, value=None, error=e
                    )
        ...
        response = DBVerifierResponse(
            result, total_messages - len(result), len(result), total_messages
        )
        return Result(
            success=True, check_success=len(result) == 0, value=response, error=None
        )
